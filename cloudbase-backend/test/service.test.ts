...
test("instant analysis creation is idempotent and reports whether VLM should be triggered", async () => {
  const repo = new MemoryRepository(); const objects = new FakeObjects();
  const api = new TaskService(repo, objects, config, () => new Date("2026-08-24T00:00:00Z"));
  const ticket = await api.issueUpload("u1", { filename: "instant.mp4", content_type: "video/mp4", size_bytes: 123, duration_seconds: 20 });

  const first = await api.createInstantJob("u1", ticket.upload_id);
  const replay = await api.createInstantJob("u1", ticket.upload_id);

  assert.equal(first.created, true);
  assert.equal(replay.created, false);
  assert.strictEqual(replay.task, first.task);
