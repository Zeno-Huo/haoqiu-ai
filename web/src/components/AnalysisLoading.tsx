...
export default function AnalysisLoading({filename,progress,stage,uploading=false,error,onRetry}:{filename:string;progress:number;stage:string;uploading?:boolean;error?:string;onRetry?:()=>void}) {
 const messages=[...tips,uploading?'视频上传完成后，自动开始分析。':'正在结合前后画面，整理球员表现。','高机位画面通常更利于判断。']
 const [tip,setTip]=useState(0)
 useEffect(()=>{if(error)return;const timer=window.setInterval(()=>setTip(n=>(n+1)%messages.length),5000);return()=>clearInterval(timer)},[error,messages.length])
 ...
 {error?<section className="vr-loading-error" role="alert"><p>{error}</p>{onRetry&&<button className="vr-primary" onClick={onRetry}>再试一次</button>}</section>:<><h2>{stage}</h2><div className="vr-reassurance vr-tip-roll"><p key={tip}>{messages[tip]}</p></div><ol className="vr-steps">{['读取视频','分析表现','生成报告'].map((s,i)=><li key={s} className={i===(uploading?0:1)?'active':i<(uploading?0:1)?'done':''}><span>{!uploading&&i===0?<VisualIcon name="check"/>:null}</span>{s}</li>)}</ol></>}
 <footer><Link className="vr-secondary" to="/">返回首页</Link></footer></main>
}
