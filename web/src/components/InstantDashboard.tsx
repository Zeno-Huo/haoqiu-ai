...
function PlayerCard({ player }: { player: InstantPlayer }) {
  ...
  return <article className="panel p-4"><header className="flex items-start justify-between gap-3 border-b border-[var(--line)] pb-3"><div><h3 className="text-base font-semibold text-[var(--text-primary)]"><span className="mr-2 font-score text-[var(--text-muted)]">{player.number || '?'}</span>{player.name || '候选球员'}</h3>{player.position && <span className="mt-2 inline-block rounded border border-[var(--line)] px-2 py-1 text-xs text-[var(--text-secondary)]">{player.position}</span>}</div><div className="text-right"><b className="font-score text-2xl text-[var(--ai)]">{value(player.score)}</b><span className="block text-[10px] text-[var(--text-muted)]">综合评分 / 10</span></div></header><div className="data-list">{statItems.map(([key, label]) => <span key={key}>{label} <b>{value(s[key])}</b></span>)}</div>{player.insights.slice(0, 3).map((item) => <p className="insight" key={item}>{item}</p>)}<EventList events={player.events} title="关联片段" /></article>
}
