...
服务对源视频逐帧执行完整检测，不抽帧、不生成智能剪辑。运行状态包含按当前处理速度估算的 `eta_seconds`；该值会随视频内容和 GPU 负载变化，不是完成时间承诺。若解码在源视频结束前异常中断，任务会以 `VIDEO_DECODE_INCOMPLETE` 失败，不会把部分结果标记成功。OpenCV 报告的容器帧数可能存在尾部索引误差，因此完整性校验最多容许2帧差异（30fps时约0.07秒），不采用按比例放宽。
